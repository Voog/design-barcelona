(function ($) {
  ...
  $(function () {
    ...
    $(".js-mobile-lang-menu-btn").click(function (event) {
      ...
      $("body")
        ...
        .addClass("mobile-lang-menu-open");
    });

    $(".js-mobile-lang-menu-close").click(function (event) {
      event.preventDefault();
      $("body")
        .removeClass("mobile-lang-menu-open")
        .addClass("mobile-main-menu-open");
    });
    ...
    $(".voog-bg-picker-btn").click(function (e) {
      e.stopPropagation();
    });
  ...
  var setScrollerArrow = function () {
    ...
    if (
      $arrow.length &&
      main.scrollWidth &&
      (main.scrollLeft || main.scrollLeft === 0)
    ) {
  ...
  var initMobileSwipe = function () {
    if (
      ...
      $(".slider-counter .slider-dot")
        .removeClass("slider-dot-active")
        .eq(swipeindex)
        .addClass("slider-dot-active");

      $(".article .link, .new-article").swipe({
        //Default is 75px, set to 0 for demo so any distance triggers swipe
        swipeLeft: function () {
          $(".main").animate(
            { scrollLeft: "+=" + $(window).width() + "px" },
            200,
            "swing",
            function () {
              swipeindex++;
  ...
  var setLayout = function (withShoppingCart) {
    ...
    if (
      ...
      $("body").hasClass("edy-checkout-button-visible")
    ) {
      mh =
        $(window).height() -
        $h.height() -
        ($(".footer").is(":hidden") ? 0 : $(".footer").innerHeight()) -
        ($("html").hasClass("editmode") ? 40 : 0) -
        41; // 41 is the height of the checkout button.
    } else {
      mh =
        $(window).height() -
        $h.height() -
    ...
    $(".header").css(
      "position",
      $(".header").height() > 150 ? "absolute" : "fixed"
    );
  ...
  (isScrolling = false), (oldie = window.oldie || false);

  // Stream
  var initScrollOverride = function () {
    if (
      $("html").hasClass("no-touchevents") &&
      $(".blog-list-page .main").length > 0 &&
      $.fn.mousewheel
    ) {
      if (oldie) {
  ...
  var handleMouseWheel = function (event, delta, x, y) {
    ...
    $(".blog-list-page .main").scrollLeft(
      $(".blog-list-page .main").scrollLeft() - dx * 3
    );
...
(function ($) {
      ...
      .each(
        $.proxy(function (i, obj) {
          ...
          if (
            $(window).scrollTop() +
              $(window).height() +
              (this.element !== window ? this.$element.scrollTop() : 0) +
              this.options.distance >
              offset.top &&
            $(window).scrollLeft() +
              $(window).width() +
              (this.element !== window ? this.$element.scrollLeft() : 0) +
              this.options.distance >
              offset.left
          ) {
  ...
  Plugin.prototype.init = function () {
    ...
    this.$element.on(
      "scroll." + this._name,
      debounce(this.load.bind(this), this.options.debounceTime)
    );
  ...
  $.fn[pluginName] = function (arg) {
    ...
    return this.each(function () {
      ...
      if (!d && (typeof arg === "object" || typeof arg === "undefined")) {
        $.data(this, pluginName, new Plugin(this, arg));
